...
CREATE TABLE IF NOT EXISTS public.pii_entities (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.redaction_methods (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL,
...
-- Insert default redaction methods if not exists
INSERT INTO public.redaction_methods (name, description, example, sort_order) 
SELECT * FROM (VALUES
...
) AS t(name, description, example, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM public.redaction_methods);
...
DROP POLICY IF EXISTS "Users can view PII entities from their documents" ON public.pii_entities;
...
DROP POLICY IF EXISTS "Users can create PII entities for their documents" ON public.pii_entities;
...
DROP POLICY IF EXISTS "Users can view processing results from their documents" ON public.processing_results;
...
DROP POLICY IF EXISTS "Users can create processing results for their documents" ON public.processing_results;
...
DROP POLICY IF EXISTS "Anyone can view redaction methods" ON public.redaction_methods;
...
CREATE INDEX IF NOT EXISTS idx_pii_entities_document_id ON public.pii_entities(document_id);
CREATE INDEX IF NOT EXISTS idx_pii_entities_type ON public.pii_entities(entity_type);
CREATE INDEX IF NOT EXISTS idx_processing_results_document_id ON public.processing_results(document_id);